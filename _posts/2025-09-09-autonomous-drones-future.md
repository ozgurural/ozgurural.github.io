...
This deterministic-safety-envelope architecture is not something we invented for 2019-vintage tactical UAVs. The pattern traces directly to the 2014 METU Clover autonomous last-mile-delivery capstone robot, where a 3-node Arduino stack mapped exactly onto the Kargu FCC/GCS split we shipped five years later. Clover's Perception node (Raspberry Pi visual-object classification) = Kargu's learned vision-tracking inference; Clover's Safety node (hard-coded Arduino MKR1000 proximity-sensor veto, 420ms fail-safe threshold against ultrasonic range readings) = Kargu's deterministic C++ FCC safety envelope; Clover's Mission node (ESP8266 waypoint state machine) = Kargu's GCS mission state machine with PBIT/CBIT harness. The 2014 94% on-bench delivery rate on METU courtyard asphalt is the first datapoint in a throughline that runs through 2019 Kargu sub-16ms GCS frame budgets, 2024 SecurePoL 10E-6 drift tolerance, 2025 Avion 50 GB/s simulator telemetry, and 2026 clinical-AI 3-hash signed measurement invariants. The envelope, the veto, and the deterministic audit trail do not change; only the domain, the sensors, and the regulator do.
