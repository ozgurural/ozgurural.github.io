...
async function vendorTarget(target) {
  ...
  const rules = [];
  let raw = 0, bytes = 0;
  // weight alone can collide (Lora is 400-italic only, 500/600-normal only,
  // so it happens not to here) but style is included in the filename so nothing
  // relies on that staying true.
  for (const { f, want } of deduped) {
    const slug = f.family.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const styleTag = f.style === 'italic' ? 'i' : '';
    const name = `${slug}-${f.weight}${styleTag}-${f.subset}.woff2`;
  ...
  console.log(`${deduped.length} face(s)${deduped.length !== kept.length ? ` (${kept.length - deduped.length} exact duplicate(s) dropped)` : ''}, ` +
