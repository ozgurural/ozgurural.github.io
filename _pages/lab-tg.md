...
<section class="lab-card lab-experiment" id="lab-tg" style="margin-top: 0;">
  ...
  <p class="lab-card__lead">⛏️ Bitcoin offers probabilistic finality without a trusted central party. Miners race to extend valid chains; nodes follow the chain with the most accumulated proof of work. To reverse a payment, an attacker must catch and overtake the honest chain, and the modeled success probability decays <strong>exponentially</strong> in the number of confirmations while the attacker controls less than half the hash power. This animation derives that probability from the ground up: the gambler's ruin, Satoshi's Poisson head-start, and the punchline that "<em>6 confirmations</em>" hides an assumption about <em>who you're racing</em>.</p>
  ...
  <details class="lab-reveal" open>
    ...
    <p><strong>Consensus is a race, not a vote.</strong> Bitcoin has no authority that decides which history is true. Nodes follow the valid chain with the most accumulated proof of work. Each block discovery is modeled as a weighted race whose win probability equals a miner's share of total hashrate, so an attacker with fraction <em>q</em> of the network mines the next block with probability <em>q</em>.</p>
