...
<section class="lab-card lab-experiment" id="lab-tmr">
  ...
  <details class="lab-reveal">
    <summary>What this shows</summary>
    <p>If hardware failures are truly independent random variables, combining three of them gives you cubic armor: $P(\text{sys fail}) = 3q^2(1-q) + q^3 \approx 3q^2$. If your processor crashes 5% of the time, three processors crash at the exact same time only 0.75% of the time. This cubic suppression is why Apollo 11 didn't crater into the Moon.</p>
    <p>But the universe hates free hardware upgrades. If those failures share a root cause—a massive solar flare, an overheated chassis, or a software bug—they become correlated ($\rho > 0$). When failures correlate perfectly ($\rho = 1$), the math collapses to $P(\text{sys fail}) = q$. You just bought three expensive computers to get the reliability of one. The critical break-even point is exactly where the math in this widget crosses $10\times$ gain. Cross that threshold, and you've built the Ariane 5 rocket launch: fully redundant computers cleanly agreeing to self-destruct.</p>
  </details>
</section>
