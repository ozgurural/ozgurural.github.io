...
seo_title: "Universal Jira: Prediction Markets, animated"
...
<section class="lab-card lab-experiment" id="lab-jira" style="margin-top: 0;">
  ...
  <p class="lab-film__legend" role="img" aria-label="Colour key: cyan=AMM curve, amber=YES price, green=developer effort, violet=oracle">
    ...
    <span><i style="background:#fbbf24"></i> "YES" share price (implied probability)</span>
  ...
  <details class="lab-reveal" open>
    ...
    <p><strong>The Coordination Problem.</strong> Traditional software uses managers to assign tasks and evaluate progress. In a trustless global network, a market can aggregate beliefs and incentives into a public coordination signal. Truth still depends on how the outcome is defined and settled.</p>
    ...
    <p><strong>Closing the loop: let the agents write the tickets.</strong> Everything above still assumes a person notices the problem and opens the market. Take that assumption away. An agent with read access to the repository, the issue tracker and production telemetry can propose the proposition itself, post it to the contract, and let the price form. The cycle then runs end to end without a human in it: an agent proposes, the market prices, an agent works, the oracle settles. This is one candidate coordination layer that could scale with the number of agents rather than the number of managers, if proposal quality and settlement can be secured.</p>
